...
namespace TransferControl.Engine
{
    public class RouteControl : AlarmMapping, ICommandReport
    {
                ...
                if (find.Count() != 0)
                {
                    ...
                    foreach (Job each in Js)
                    {
                        ...
                        foreach (Path eachPath in PathManagement.GetFinishPath(ScriptName, "WAIT_PROCESS", ""))
                        {
                            ...
                            {
                                //第二台ALIGNER強制不做GetWait
                                if (each.ProcessNode.Equals(RobotNode.AlternativeAligner) && eachAction.Method.Equals(Transaction.Command.RobotType.GetWait))
                                {
                                    continue;
                                }
                ...
                switch (Action.EqpType)
                {
                    case "Robot":
                        ...
                        while (true)//等待主控權
                        {
                            ...
                            {
                                if (Node.LastFinMethod.Equals(Transaction.Command.RobotType.WaitBeforePut))
                                {
                                    Node.PutOut = false;
                                }
                    ...
                    case "NextRobot":
                        ...
                        while (true)//第二段 等待主控權
                        {
                            ...
                            //logger.Debug(JsonConvert.SerializeObject(Node));
                            logger.Debug(Node.Name + " 等待Robot主控權 :" + Node.GetMutex + "," + Node.GetAvailable + "," + Node.InterLock + "," + Node.UnLockByJob + "," + TargetJob.Job_Id + "," + Node.JobList.Count);
                            SpinWait.SpinUntil(() => GetNextRobotControl(Node, TargetJob, Force, Action), SpinWaitTimeOut); //等待Robot有空
                            if (_Mode.Equals("Stop"))
                            {
                                logger.Debug("離開自動模式");
                                return;
                            ...
                            lock (Node)
                            {
                                ...
                                else if (Node.Phase.Equals("2") && ((Node.GetAvailable && Node.GetMutex && Node.JobList.Count < 2) || (Node.GetAvailable && !Node.InterLock && (Node.UnLockByJob.Equals(TargetJob.Job_Id) || Node.UnLockByJob.Equals("")) && Node.JobList.Count < 2) || Force))
                                {
                                    ...
                                    if (!Action.Method.Equals(Transaction.Command.RobotType.GetWait))
                                    {
        ...
        private bool GetNextRobotControl(Node Node, Job TargetJob, bool Force, Path.Action Action)
        {
                                    ...
                                    {//還沒做完
                                        ...
                                        {//做完但沒通過檢查
                                            if (_HostReport != null)
                                            {
                                                ...
                                                _HostReport.On_TaskJob_Aborted(Txn.FormName, ErrorMessage);
                                            }
                                        }
                                        //檢查到不是Task，不做事
                                    }
                        ...
                        case "ROBOT":
                            ...
                            if (!_Mode.Equals("Stop") && Txn.FormName.Equals("Running"))
                            {
                                switch (Node.Phase)
                                {
                                    ...
                                    case "2":
                                        //進入處理階段
                                        if (Node.JobList.Count == 0 && Node.WaitForCarryCount == 0 && (Txn.Method.Equals(Transaction.Command.RobotType.Put) || Txn.Method.Equals(Transaction.Command.RobotType.PutBack)))
                                        {
        ...
        public void On_Command_TimeOut(Node Node, Transaction Txn)
        {
            ...
            if (_HostReport != null)
            {
                _HostReport.On_TaskJob_Aborted(Txn.FormName, "TimeOut");
            }
        ...
        public void On_Command_Error(Node Node, Transaction Txn, ReturnMessage Msg)
        {
            ...
            if (_HostReport != null)
            {               
                _HostReport.On_TaskJob_Aborted(Txn.FormName, Msg.Value);
            }
